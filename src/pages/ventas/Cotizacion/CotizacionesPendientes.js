...
const CotizacionRow = ({cotizacion, index, handleModalCotizacion, setSelected, setView}) => {
	...
	const AgregarVentaDeNuevo = () =>{
		...
		let objectToSave= JSON.stringify({
			cliente: (cotizacion.cliente.nombre_cliente === null) ? {} : cotizacion.cliente,
		...
		if (datosVenta === null) {
			...
			if(moment(cotizacion.fecha_vencimiento_cotizacion).isBefore(moment().format('YYYY MM DD')) ){
				setOpenMessage({message: 'La cotización seleccionada ya tiene la fecha vencida.', open:true});
				return;
			}
			...
			setOpenMessage({message: 'No puedes agregar una venta cuando ya está una en curso.', open:true});
		}
		} catch (error) {
...
const RegresarVenta = ({ openMessage, handleClickOpen, AgregarVentaDeNuevo }) => {
	return (
	  <Fragment>
		<IconButton
		  aria-label="regresar"
